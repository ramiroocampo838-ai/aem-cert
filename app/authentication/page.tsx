/**
 * Authentication in AEM Presentation - Main Page
 * Interactive slide-based presentation on Adobe IMS, SAML 2.0, and token-based authentication.
 * Architecture mirrors app/env-setup/page.tsx — independent content source.
 */
...
import { authenticationSlides, getTotalAuthenticationSlides } from "@/lib/authentication-slides"
import type { ModalContent } from "@/lib/authentication-slides"

function AuthenticationContent() {
  const router = useRouter()
  const searchParams = useSearchParams()
  const totalSlides = getTotalAuthenticationSlides()
  ...
  const currentSlideData = authenticationSlides[currentSlide - 1]

  const timeRemaining = authenticationSlides
    .slice(currentSlide)
    .reduce((total, slide) => total + slide.estimatedTime, 0)

  const slidesTitles = authenticationSlides.map(s => s.title)
            ...
            <div className="flex flex-wrap gap-2">
              {currentSlideData.modals.map((modal, idx) => (
                <button
                  ...
                  className="rounded-lg border border-cyan-400/40 bg-cyan-500/10 px-4 py-2 text-sm text-cyan-300 transition-colors hover:bg-cyan-500/20"
                >
                  📖 {modal.title}
                </button>
              ))}
