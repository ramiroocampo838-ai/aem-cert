/**
 * AEM Environment Setup Presentation - Main Page
 * Interactive slide-based presentation on local AEM development environment.
...
import { envSetupSlides, getTotalEnvSetupSlides } from "@/lib/env-setup-slides"
import type { ModalContent } from "@/lib/env-setup-slides"

function EnvSetupContent() {
  const router = useRouter()
  const searchParams = useSearchParams()
  const totalSlides = getTotalEnvSetupSlides()
  ...
  const currentSlideData = envSetupSlides[currentSlide - 1]

  const timeRemaining = envSetupSlides
    .slice(currentSlide)
    .reduce((total, slide) => total + slide.estimatedTime, 0)

  const slidesTitles = envSetupSlides.map(s => s.title)
            ...
            <div className="flex flex-wrap gap-2">
              {currentSlideData.modals.map((modal, idx) => (
                <button
                  ...
                  className="rounded-lg border border-violet-400/40 bg-violet-500/10 px-4 py-2 text-sm text-violet-300 transition-colors hover:bg-violet-500/20"
                >
                  📖 {modal.title}
                </button>
              ))}
