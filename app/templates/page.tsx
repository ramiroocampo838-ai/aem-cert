/**
 * AEM Templates & Client Libraries Presentation - Main Page
 * Interactive slide-based presentation on components, editable templates,
 * experience fragments, and client libraries.
 * Architecture mirrors app/authentication/page.tsx — independent content source.
 */
...
import { templatesSlides, getTotalTemplatesSlides } from "@/lib/templates-slides"
import type { ModalContent } from "@/lib/templates-slides"

function TemplatesContent() {
  const router = useRouter()
  const searchParams = useSearchParams()
  const totalSlides = getTotalTemplatesSlides()
  ...
  const currentSlideData = templatesSlides[currentSlide - 1]

  const timeRemaining = templatesSlides
    .slice(currentSlide)
    .reduce((total, slide) => total + slide.estimatedTime, 0)

  const slidesTitles = templatesSlides.map(s => s.title)
            ...
            <div className="flex flex-wrap gap-2">
              {currentSlideData.modals.map((modal, idx) => (
                <button
                  ...
                  className="rounded-lg border border-amber-400/40 bg-amber-500/10 px-4 py-2 text-sm text-amber-300 transition-colors hover:bg-amber-500/20"
                >
                  📖 {modal.title}
                </button>
              ))}
